...
// HostHostDaemonSetsController is responsible for synchronizing DaemonSet objects stored
// in the system with actual running pods.
type HostDaemonSetsController struct {
...
func NewHostDaemonSetsController(
	...
	kubeClient clientset.Interface,
	failedPodsBackoff *flowcontrol.Backoff,
) (*HostDaemonSetsController, error) {
	eventBroadcaster := record.NewBroadcaster()

	dsc := &HostDaemonSetsController{
	...
	// nolint:errcheck
	daemonSetRefInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc:    dsc.addDaemonset,
		UpdateFunc: dsc.updateDaemonset,
		DeleteFunc: dsc.deleteDaemonset,
	})
	dsc.dsLister = daemonSetRefInformer.Lister()
	dsc.dsStoreSynced = daemonSetRefInformer.Informer().HasSynced
...
func (dsc *HostDaemonSetsController) Run(ctx context.Context, workers int) {
...
func (dsc *HostDaemonSetsController) processNextWorkItem(ctx context.Context) bool {
...
func (dsc *HostDaemonSetsController) enqueueDaemonSetAfter(obj interface{}, after time.Duration) {
...
func (dsc *HostDaemonSetsController) addHistory(obj interface{}) {
...
func (dsc *HostDaemonSetsController) updateHistory(old, cur interface{}) {
...
func (dsc *HostDaemonSetsController) deleteHistory(obj interface{}) {
...
func (dsc *HostDaemonSetsController) addPod(obj interface{}) {
	pod := obj.(*v1.Pod)
...
func (dsc *HostDaemonSetsController) updatePod(old, cur interface{}) {
	curPod := cur.(*v1.Pod)
	oldPod := old.(*v1.Pod)
...
func (dsc *HostDaemonSetsController) deletePod(obj interface{}) {
	pod, ok := obj.(*v1.Pod)
...
func (dsc *HostDaemonSetsController) addNode(obj interface{}) {
...
func (dsc *HostDaemonSetsController) updateNode(old, cur interface{}) {
	oldNode := old.(*v1.Node)
	curNode := cur.(*v1.Node)
...
func (dsc *HostDaemonSetsController) updateDaemonSetStatus(ctx context.Context, ds *kosmosv1alpha1.ShadowDaemonSet, nodeList []*v1.Node, hash string, updateObservedGen bool) error {
	...
	err = storeDaemonSetStatus(ctx, dsc.kosmosClient, ds, desiredNumberScheduled, currentNumberScheduled, numberMisscheduled, numberReady, updatedNumberScheduled, numberAvailable, numberUnavailable, updateObservedGen)
	if err != nil {
...
func (dsc *HostDaemonSetsController) syncDaemonSet(ctx context.Context, key string) error {
...
func getPodDaemonSets(pod *v1.Pod, lister kosmoslister.ShadowDaemonSetLister) ([]*kosmosv1alpha1.ShadowDaemonSet, error) {
	...
	for i := range list {
		...
		// If a daemonSetRef with a nil or empty selector creeps in, it should match nothing, not everything.
		if selector.Empty() || !selector.Matches(labels.Set(pod.Labels)) {
			continue
		}
		daemonSets = append(daemonSets, daemonSetRef)
	}

	if len(daemonSets) == 0 {
		return nil, fmt.Errorf("could not find daemon set for pod %lister in namespace %lister with labels: %v", pod.Name, pod.Namespace, pod.Labels)
	}
